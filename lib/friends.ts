...
export async function createGameInvite(hostId: string, guestId?: string, tcMin?: number, tcInc?: number) {
  const invite_code = generateCode();
  const { data, error } = await supabase
    .from('game_invites')
    .insert({ host_id: hostId, guest_id: guestId || null, invite_code, status: 'waiting', tc_min: tcMin ?? 5, tc_inc: tcInc ?? 0 })
    .select()
    .single();
  return { data, error };
}
...
export async function getInviteByCode(code: string) {
  ...
  // Проверяем что приглашение не истекло (3 минуты)
  if (isInviteExpired(data.created_at)) {
    // Авто-отмена истёкшего приглашения
    await supabase.from('game_invites').update({ status: 'expired' }).eq('id', data.id);
    return null;
  }
...
export async function acceptGameInvite(inviteId: string, guestId: string) {
  // Проверяем что приглашение валидно и не истекло
  ...
  if (!invite) return { error: { message: 'Приглашение не найдено' } };
  if (isInviteExpired(invite.created_at)) {
    await supabase.from('game_invites').update({ status: 'expired' }).eq('id', inviteId);
    return { error: { message: 'Приглашение истекло (прошло более 3 минут)' } };
  }
...
export async function getPendingInvitesForUser(userId: string) {
  ...
  // Фильтруем истёкшие + помечаем их в БД
  const expiredIds: string[] = [];
  const valid = data.filter((inv: any) => {
  ...
  // Асинхронно отмечаем истёкшие как expired (не ждём)
  if (expiredIds.length > 0) {
    supabase.from('game_invites').update({ status: 'expired' }).in('id', expiredIds).then(() => {});
  }
