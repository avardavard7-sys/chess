...
export async function analyzeGame(
  ...
  for (let i = 0; i < movesJson.length; i++) {
    ...
    // eval_after с точки зрения стороны, чей был ход
    // Stockfish всегда оценивает с точки зрения стороны, чей сейчас ход
    // После нашего хода — ходит оппонент, значит eval нужно инвертировать
    const evalAfterFromPlayer = -afterResult.eval;

    // Потеря = лучший eval - eval после нашего хода (с точки зрения игрока)
    const evalBestFromPlayer = color === 'w' ? bestEval : -bestEval;
    const evalActualFromPlayer = color === 'w' ? evalAfterFromPlayer : -evalAfterFromPlayer;

    // Упрощаем: drop = разница между лучшим eval и реальным
    const evalDrop = Math.max(0, evalBestFromPlayer - evalActualFromPlayer);
    ...
    analyzedMoves.push({
      ...
      eval_after: evalAfterFromPlayer,
