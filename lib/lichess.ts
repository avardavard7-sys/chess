...
// Очищаем старые токены darkhorser которые остались от админского NETWORK_TOKEN
function cleanupOldDarkhorserToken() {
  if (typeof window === 'undefined') return;
  try {
    const userJson = localStorage.getItem('net_user');
    if (userJson) {
      const user = JSON.parse(userJson);
      // Если в кеше остался darkhorser — это старый админский токен, чистим
      if (user?.username === 'darkhorser' || user?.username === 'Dark Horse') {
...
export function getToken(): string | null {
  ...
  cleanupOldDarkhorserToken();
  // ТОЛЬКО личный токен пользователя — общий админский удалён
