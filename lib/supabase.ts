...
// createBrowserClient из @supabase/ssr САМ управляет хранилищем через cookies
// и автоматически синхронизируется с middleware.
// НЕ передавать auth опции — они конфликтуют с SSR cookies management!
// Все auth опции (persistSession, autoRefreshToken, detectSessionInUrl)
// уже включены по умолчанию через cookies.
...
export async function updateElo(userId: string, newElo: number, result: 'win' | 'loss' | 'draw', mode?: string) {
  const { data: profile } = await getProfile(userId);
  if (!profile) return;

  // Тренеры и админы НЕ теряют ELO при игре с AI
  // Но в онлайн/против друга — обычное правило
  const isTrainerOrAdmin = profile.is_trainer || profile.is_admin;
  if (isTrainerOrAdmin && mode === 'ai') {
    // Только обновляем счётчик игр, ELO не трогаем
