...
export default function LichessGame({
  ...
  const handleGameEnd = useCallback((status: string, winner?: string) => {
    ...
    setGameEnded(true);
    // Очищаем premove при окончании партии
  ...
  // Локальное тикание часов между серверными обновлениями
  // Lichess присылает wtime/btime только после хода — между ходами часы должны тикать сами
  useEffect(() => {
    if (gameEndedRef.current) return;
    // Не тикаем пока партия не началась (нет ходов = первый ход белых)
    // Но если ходы уже были — тикаем у того кто сейчас ходит
    const movesPlayed = chess.history().length;
    if (movesPlayed === 0) return; // Ждём первый ход

    const isWhiteTurn = chess.turn() === 'w';
    const intervalId = setInterval(() => {
      if (gameEndedRef.current) return;
      if (isWhiteTurn) {
        setWtime(prev => Math.max(0, prev - 100));
      } else {
        setBtime(prev => Math.max(0, prev - 100));
      }
    }, 100);
  ...
  const executeMove = useCallback(async (from: Square, to: Square, promotion?: string) => {
    ...
    // Оптимистичное обновление — применяем ход локально сразу
    // Stream от Lichess потом синхронизирует состояние
    chess.move({ from, to, promotion: (promotion || 'q') as 'q' | 'r' | 'b' | 'n' });
    setFen(chess.fen());

    const ok = await sendMove(token, gameId, uciMove);
    if (!ok) {
      // Откатываем ход если отправка не удалась
      chess.undo();
      setFen(chess.fen());
      triggerShake();
    }
  ...
  // Автовыполнение premove когда стал мой ход (после executeMove т.к. использует его)
  useEffect(() => {
    if (gameEndedRef.current) return;
    ...
    // Проверяем валидность premove после хода соперника
    setTimeout(() => {
      try {
        const testChess = new Chess(chess.fen());
        const valid = testChess.move({ from: pm.from, to: pm.to, promotion: 'q' });
        if (valid && !isPromotion(chess.fen(), pm.from, pm.to)) {
  ...
  }, [fen, chess, isMyTurn, executeMove]);

  const onPieceDrop = useCallback((sourceSquare: Square, targetSquare: Square) => {
    // Premove — на ходу соперника сохраняем намерение
    if (!isMyTurn()) {
      if (gameEndedRef.current) return false;
      premoveRef.current = { from: sourceSquare, to: targetSquare };
      setPremoveSquares({ from: sourceSquare, to: targetSquare });
      setFen(chess.fen()); // принудительно возвращаем фигуру визуально
      return false;
    }

    // Сначала проверяем валидность хода — chess.js v1 бросает исключение
    let valid = null;
    try {
      const testChess = new Chess(chess.fen());
      valid = testChess.move({ from: sourceSquare, to: targetSquare, promotion: 'q' });
    } catch {
      valid = null;
    }
    ...
    if (!valid) {
      triggerShake();
      // Принудительно перерисовываем доску чтобы фигура вернулась
      setFen(chess.fen());
      return false;
    }
  ...
  const handleSquareClick = useCallback(async (square: Square) => {
    // На ходу соперника — отмена premove
    if (!isMyTurn()) {
    ...
    <div className="flex flex-col lg:flex-row gap-6 w-full max-w-5xl mx-auto">
        ...
        <div className="glass p-3 rounded-xl flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {opponentRating && <div className="text-xs text-white/40">{t('daily_rating')} {displayOpponentRating}</div>}
            </div>
          </div>
          <div
        ...
        <motion.div className={`relative ${shaking ? 'shake' : ''}`}>
          <Chessboard
            ...
            isDraggablePiece={({ piece }) => {
              // Свой цвет всегда таскаем (для premove на ходу соперника)
              return piece[0] === (myColor === 'white' ? 'w' : 'b');
            }}
          />
        </motion.div>
        ...
        <div className="glass p-3 rounded-xl flex items-center justify-between mt-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {myRating && <div className="text-xs text-white/40">{t('daily_rating')} {myRating}</div>}
            </div>
          </div>
          <div
            className={`font-mono font-bold text-xl px-3 py-1 rounded-lg ${
      ...
      <div className="flex-1 flex flex-col gap-4 min-w-0">
        ...
        <div className="glass p-4 rounded-xl">
          <div className="text-xs text-white/50 mb-2 uppercase tracking-wider">{t('captured_pieces')}</div>
          <div className="flex items-start gap-1 mb-1">
            <span className="text-xs text-white/40 w-16 mt-0.5">{t('analysis_white')}:</span>
            <span className="text-base leading-tight flex-1">{capturedPieces.white.join('') || '–'}</span>
          </div>
          <div className="flex items-start gap-1">
            <span className="text-xs text-white/40 w-16 mt-0.5">{t('analysis_black')}:</span>
        ...
        <div className="glass p-4 rounded-xl flex-1 flex flex-col">
          ...
          <div ref={moveHistoryRef} className="overflow-y-auto" style={{ maxHeight: '220px' }}>
            {moveHistory.length === 0 ? (
              <p className="text-white/25 text-sm italic">{t('no_moves_yet')}</p>
            ) : (
        ...
        {promotionSquare && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70"
            ...
            <motion.div className="glass p-6 rounded-2xl text-center"
              ...
              <h3 className="text-xl font-bold mb-5 playfair text-yellow-400">{t('pawn_promotion')}</h3>
