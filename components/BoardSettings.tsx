    ...
    <>
      ...
      {isOpen && (
        <motion.div className="absolute bottom-full right-0 mb-2 glass p-4 rounded-xl z-50 w-72"
          ...
          <div className="text-xs text-white/40 uppercase tracking-wider mb-3">Тема доски</div>
          ...
          <div className="text-[10px] text-white/20 mt-2 text-center">Текущая: {BOARD_THEMES.find(t => t.id === current)?.name}</div>
        </motion.div>
      )}
    </>
  );
