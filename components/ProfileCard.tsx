    ...
    <motion.div className="glass p-5 rounded-2xl mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }}>
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="text-xs text-white/40 mb-1">Мой тренер</div>
          <span className="text-sm text-white/30">Тренеры пока не добавлены</span>
        </div>
      </div>
    </motion.div>
  );
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <select value={selected} onChange={async (e) => {
            ...
            // 2. Создаём/обновляем запись в school_students (чтобы тренер видел ученика)
            if (val) {
...
export default function ProfileCard({ profile, gameHistory, onProfileUpdate }: ProfileCardProps) {
  ...
  useEffect(() => {
    supabase.from('school_students').select('rating').eq('profile_id', profile.id).maybeSingle().then(({ data }) => {
      if (data) {
        const RANKS = [{ min: 0, name: 'Без разряда', color: '#6b7280' }, { min: 300, name: '5 разряд', color: '#CD7F32' }, { min: 500, name: '4 разряд', color: '#3b82f6' }, { min: 1000, name: '3 разряд', color: '#8b5cf6' }, { min: 1150, name: '2 разряд', color: '#f59e0b' }, { min: 1300, name: '1 разряд', color: '#ef4444' }];
            ...
            <div className="flex items-center gap-6 mb-6 flex-wrap">
              ...
              {internalRating && (
                <div className="inline-flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs text-white/40 uppercase tracking-wider">Внутренний</div>
              ...
              <div className="mb-4">
                {editingRealElo ? (
                  <div className="flex items-center gap-2">
                    ...
                    }} className="px-3 py-2 rounded-xl text-xs font-semibold text-black bg-yellow-400">Сохранить</button>
                ...
                return (
                  <>
                    <div className="flex justify-between text-xs text-white/40">
                      <span>{prev}</span>
                      <span>До следующего ранга: {next - effectiveElo} ELO</span>
                      <span>{next}</span>
                    </div>
