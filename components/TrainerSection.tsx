  ...
  return (
    <motion.div className="glass p-5 rounded-2xl mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>📚 {t('from_trainer')}</h2>
        {trainerProfile && (
      ...
      <div className="flex items-center gap-3 mb-4 p-3 rounded-xl bg-white/5">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-white/30">{t('your_trainer')}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="p-2 rounded-lg bg-white/5 text-center">
          ...
          <div className="text-[10px] text-white/30">{t('points_label')}</div>
        </div>
        <div className="p-2 rounded-lg bg-white/5 text-center">
          <div className="text-lg font-bold" style={{ color: attPct >= 80 ? '#4ade80' : attPct >= 50 ? '#fbbf24' : attPct >= 0 ? '#f87171' : '#6b7280' }}>
            {attPct >= 0 ? `${attPct}%` : '—'}
          </div>
          <div className="text-[10px] text-white/30">{t('attendance_label')}</div>
        </div>
        <div className="p-2 rounded-lg bg-white/5 text-center">
          <div className="text-lg font-bold text-blue-400">{hwDone}/{hwTotal}</div>
          <div className="text-[10px] text-white/30">{t('homework_short')}</div>
        </div>
      </div>
      ...
      {completedHw.length > 0 && (
        <div className="mb-4">
          <div className="text-xs text-white/20 mb-1">{t('completed_label')} ({completedHw.length})</div>
