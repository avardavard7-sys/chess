...
export default function OnlineMatchmaking() {
  ...
  const startNetworkSearch = useCallback(async () => {
    ...
    // TIMEOUT: если за 90 сек не нашли соперника — останавливаем поиск
    searchTimeoutRef.current = setTimeout(() => {
      if (!gameFoundRef.current) {
        clearTimers();
        abortRef.current?.abort();
        setState('idle');
        alert('⏰ Соперник не найден за 90 секунд. Попробуйте позже или сыграйте против ИИ.');
      }
    }, 90000);
  }, []);
  ...
  const handleGameEnd = async (result: 'win' | 'loss' | 'draw', opponentNameArg?: string, oppRating?: number, movesData?: Array<{ from: string; to: string; san: string; promotion?: string }>) => {
    abortRef.current?.abort();
    ...
    if (localUser) {
      ...
      await updateElo(localUser.id, newElo, result, 'online');

      // Save game history с ходами для анализа
      await supabase.from('game_history').insert({
        ...
        elo_after: newElo,
        elo_change: change,
        moves_json: movesData || [],
        mode: 'online',
        player_color: myColor,
        opponent_name: opponentNameArg || opponentName || 'Соперник онлайн',
      });
      ...
      <div className="min-h-screen pt-24 pb-12 px-4">
        ...
        <div className="flex justify-center">
          <LichessGame
            ...
            onGameEnd={(result, _reason, moves) => {
              handleGameEnd(result, opponentName, opponentRating, moves);
            }}
          />
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {state === 'found' && (
            <motion.div key="found" className="glass p-10 rounded-2xl text-center"
              ...
              <p className="text-white/60">{t('starting_game')}</p>
              <div className="mt-4 flex justify-center gap-2">
                <div className={`w-5 h-5 rounded-full border-2 border-white/20 ${myColor === 'white' ? 'bg-white' : 'bg-gray-900'}`} />
                <span className="text-sm text-white/60">Вы играете {myColor === 'white' ? t('you_play_white') : t('you_play_black')}</span>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="idle" className="glass p-8 rounded-2xl text-center"
              ...
              <div className="mb-6 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm text-white/50">{t("player_network")}</span>
                  {networkConnected ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-green-400 font-semibold">{t('connected')}</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-1.5">
                      <div className="w-2.5 h-2.5 rounded-full bg-white/20" />
                      <span className="text-xs text-white/30">{t('not_connected')}</span>
                    </div>
                  )}
                </div>
                ...
                {networkConnected && netUser ? (
                  <div className="text-left">
                    <div className="text-xs text-white/40 mb-1">{t('your_account')}</div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-white/80">{netUser.username}</span>
                      <span className="text-xs text-yellow-400">{t('daily_rating')} {netUser.rating}</span>
                    </div>
              ...
              {!networkConnected && isLoggedIn && (
                <div className="flex items-center justify-center gap-2 mb-5">
                  <span className="text-xs text-white/40">{t("local_rating")}:</span>
              ...
              {!isLoggedIn && !networkConnected && (
                <div className="mb-5">
                  <p className="text-white/40 text-sm mb-3">{t('sign_in_for_rating')}</p>
                  <motion.button
