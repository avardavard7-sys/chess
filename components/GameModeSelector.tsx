...
const modes = [
  ...
  },
  {
    ...
    desc: 'Пригласите друга по ссылке или из списка друзей — играйте на разных устройствах',
...
export default function GameModeSelector({ difficulty, onBack }: GameModeSelectorProps) {
  ...
  const modeTranslations: Record<string, { title: string; desc: string }> = {
    ai: { title: t('mode_ai'), desc: t('mode_ai_desc') },
    local: { title: t('mode_local'), desc: t('mode_local_desc') },
    friend: { title: t('mode_friend'), desc: t('mode_friend_desc') },
    online: { title: t('mode_online'), desc: t('mode_online_desc') },
  };
    ...
    <div className="min-h-screen pt-24 pb-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-white/40 mt-1">{t('mode_select')}</p>
        </motion.div>
          ...
          {modes.map((mode, i) => (
            ...
            >
              ...
              <h2
                ...
                style={{ fontFamily: "'Playfair Display', serif" }}
              >
                {modeTranslations[mode.id]?.title || mode.title}
              </h2>

              {/* Description */}
              <p className="text-sm text-white/55 relative z-10 leading-relaxed">
                {modeTranslations[mode.id]?.desc || mode.desc}
              </p>
