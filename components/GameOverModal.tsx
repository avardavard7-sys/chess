...
export default function GameOverModal({ isOpen, result, eloChange, coinChange, reason, gameId, onPlayAgain }: GameOverModalProps) {
  ...
  const resultConfig = {
    win: {
      icon: '🏆',
      title: t('game_over_win'),
      subtitle: t('game_over_win_sub'),
      color: '#f59e0b',
    ...
    loss: {
      icon: '😔',
      title: t('game_over_loss'),
      subtitle: t('game_over_loss_sub'),
      color: '#f87171',
    ...
    draw: {
      icon: '🤝',
      title: t('game_over_draw'),
      subtitle: t('game_over_draw_sub'),
      color: '#94a3b8',
          ...
          >
            ...
            {coinChange !== null && coinChange !== undefined && (
              ...
              >
                ...
                <span className="text-white/60 text-sm">{t('coins_label')}</span>
              </motion.div>
            )}

            {reason && (
              <p className="text-white/40 text-sm mb-6">
                {reason === 'checkmate' && t('reason_checkmate')}
                {reason === 'stalemate' && t('reason_stalemate')}
                {reason === 'draw' && t('reason_draw_agree')}
                {reason === 'resigned' && t('reason_resigned')}
                {reason === 'threefold' && t('reason_threefold')}
                {reason === 'insufficient' && t('reason_insufficient')}
                {reason === 'timeout' && t('reason_timeout')}
                {reason === 'abandoned' && t('reason_abandoned')}
                {reason === 'illegal' && t('reason_illegal')}
              </p>
            )}
            ...
            {gameId && (
              <motion.button
                ...
                className="w-full mt-3 py-3 px-5 rounded-xl font-semibold border border-purple-500/30 text-purple-400 hover:bg-purple-500/10 transition-colors"
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
              >
                🔬 {t('analyze_game')}
