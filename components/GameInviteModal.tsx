...
export default function GameInviteModal({ userId, friends, isOpen, onClose, onInviteCreated }: GameInviteModalProps) {
  ...
  const inviteFriend = async (friendId: string) => {
    if (waitingFor || inviteSent) return;
  ...
  useEffect(() => { if (!isOpen) { setInviteCode(null); setInviteId(null); setCopied(false); setWaitingFor(null); setInviteSent(false); } }, [isOpen]);
        ...
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)' }}
          ...
          <motion.div className="w-full max-w-md mx-4 rounded-2xl overflow-hidden" style={{ background: 'rgba(15,23,42,0.98)', border: '1px solid rgba(245,158,11,0.3)' }}
            ...
            <div className="p-5 border-b border-white/10 flex items-center justify-between">
              <h2 className="text-lg font-bold" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>{t('invite_title')}</h2>
            ...
            {!inviteCode && (
              <div className="px-5 pt-4">
                <div className="text-xs text-white/40 mb-2">{t('invite_timing')}:</div>
            ...
            <div className="flex border-b border-white/10 mt-3">
              {(['link', 'friend'] as const).map((tabId) => (
                <button key={tabId} onClick={() => setTab(tabId)}
                  className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === tabId ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-white/40 hover:text-white/60'}`}>
                  {tabId === 'link' ? t('invite_by_link') : t('invite_friend')}
                </button>
              ))}
            </div>
                ...
                <div className="text-center">
                  {!inviteCode ? (
                    <motion.button onClick={createInviteLink} disabled={creating} className="w-full py-4 rounded-xl font-bold text-black"
                      ...
                      {creating ? t('loading') : `Создать ссылку (${tc.label})`}
                    </motion.button>
                  ) : (
                    <div className="space-y-4">
                      <div className="text-4xl font-mono font-bold text-yellow-400 tracking-widest">{inviteCode}</div>
                      <div className="text-xs text-white/30">{t('invite_timing')}: {tc.label} ({tc.type})</div>
                      <div className="flex items-center gap-2 px-4 py-3 rounded-xl cursor-pointer hover:bg-white/5 transition-colors"
                        ...
                        <span className="text-xs text-yellow-400 font-semibold flex-shrink-0">{copied ? t('invite_copied') : t('invite_copy')}</span>
                      </div>
                      <p className="text-white/30 text-xs">{t('invite_waiting')}</p>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {inviteSent ? (
                    <div className="text-center py-6">
                      ...
                      <div className="text-sm font-semibold text-green-400 mb-1">{t('invite_sent')}</div>
                      <div className="text-xs text-white/30 mb-3">{t('invite_sent_waiting')}</div>
                      <motion.div className="text-3xl" animate={{ rotate: 360 }} transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }}>♞</motion.div>
                    </div>
                  ) : friends.length === 0 ? (
                    <p className="text-center text-white/30 text-sm py-8">{t('friends_none')}</p>
                  ) : friends.map((f) => (
                    <div key={f.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <motion.button onClick={() => inviteFriend(f.id)} disabled={!!waitingFor}
                        ...
                        {waitingFor === f.id ? t('friends_invite_sending') : `Пригласить (${tc.label})`}
                      </motion.button>
                    </div>
                  ))}
                </div>
