...
interface ChessBoardProps {
  ...
  tournamentId?: string;
  onGameOver?: (result: 'win' | 'loss' | 'draw', eloChange: number, coinChange?: number) => void;
}
...
export default function ChessBoard({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // НЕ закрываем игру при React unmount — пользователь может вернуться!
    };
  }, []);
  ...
  const handleGameEnd = useCallback(async (result: 'win' | 'loss' | 'draw') => {
    ...
    if (user) {
      ...
      // Берём ходы из лога (более надёжно чем chess.history)
      ...
      onGameOver?.(result, change, earnedCoins);
    } else {
  ...
  const onPieceDrop = useCallback((sourceSquare: Square, targetSquare: Square) => {
    if (!isMyTurn()) {
      // Premove — ставим ход заранее пока соперник думает
    ...
    if (isPromotion(chess.fen(), sourceSquare, targetSquare)) {
      ...
      // Return false but reset position visually
      const currentFen = chess.fen();
      setFen('');
      requestAnimationFrame(() => setFen(currentFen));
      return false;
    ...
    if (!testMove) {
      triggerShake();
      // Force board to snap piece back by resetting FEN
      const currentFen = chess.fen();
      setFen('');
      requestAnimationFrame(() => setFen(currentFen));
      return false;
  ...
  const handleTimeOut = useCallback((color: 'white' | 'black') => {
    ...
    setGameOver({ status: 'timeout', winner: color === 'white' ? 'black' : 'white' });
    handleGameEnd(result);
  }, [playerColor, handleGameEnd]);

  const clockRunning = !gameOver && !isThinking && moveHistory.length > 0;
      ...
      <div className="flex-shrink-0">
        ...
        <div className="flex gap-2 mt-4">
          ...
          <motion.button onClick={() => {
              ...
              // Возвращаем 1 ход (или 2 для AI — свой + бота)
          ...
          <motion.button onClick={async () => {
              if (mode === 'ai' || mode === 'local') {
                gameOverRef.current = true; setGameOver({ status: 'draw' }); await handleGameEnd('draw');
              } else {
                setDrawOffered(true);
