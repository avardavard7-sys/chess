...
export default function TrainPage() {
  ...
  useEffect(() => {
    const update = () => setBoardWidth(Math.min(window.innerWidth < 640 ? window.innerWidth - 32 : 400, 440));
          ...
          {phase === 'select' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="text-center mb-8">
                ...
                <p className="text-white/40">Играй и получай подсказки в реальном времени</p>
              </div>
          ...
          {phase === 'time' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="text-center mb-4">
                ...
                <h2 className="text-xl font-bold mt-2" style={{ fontFamily: "'Playfair Display', serif" }}>Играем с {selectedCoach?.name}</h2>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="glass rounded-2xl flex flex-col h-[200px] lg:h-[460px]">
                  <div className="p-3 border-b border-white/10 flex items-center gap-2">
                    ...
                    {accuracy.total > 0 && (
                      <span className="ml-auto text-xs text-white/30">Точность: {Math.round((accuracy.good / accuracy.total) * 100)}%</span>
                    )}
                  </div>
