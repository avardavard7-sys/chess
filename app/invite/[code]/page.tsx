...
function InviteContent({ code }: { code: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (inv.host_id === user.id) {
        router.push(`/game/friend?mode=friend&session=${inv.id}&color=white&tc_min=${inv.tc_min ?? 5}&tc_inc=${inv.tc_inc ?? 0}`);
        return;
      }
      setInvite(inv);
      setStatus('ready');
  ...
  const tcMin = invite?.tc_min ?? 5;
  const tcInc = invite?.tc_inc ?? 0;
  ...
  const handleAccept = async () => {
    ...
    router.push(`/game/friend?mode=friend&session=${invite.id}&color=black&tc_min=${tcMin}&tc_inc=${tcInc}`);
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <motion.div className="glass p-10 rounded-2xl text-center max-w-md w-full" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h2 className="text-2xl font-bold mb-3" style={{ fontFamily: "'Playfair Display', serif", color: '#f87171' }}>{t('invite_expired')}</h2>
          <p className="text-white/50 mb-6">{t('invite_expired_desc')}</p>
  ...
  if (status === 'error') {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <motion.div className="glass p-10 rounded-2xl text-center max-w-md w-full" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="text-6xl mb-4">👤</div>
          <h2 className="text-2xl font-bold mb-3" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>{t('sign_in_required')}</h2>
          <p className="text-white/50 mb-6">{t('sign_in_for_invite')}</p>
          <motion.button
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <motion.div className="glass p-10 rounded-2xl text-center max-w-md w-full" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
        ...
        <p className="text-white/50 mb-2">{t('invite_to_game')}</p>
        <p className="text-sm text-yellow-400/70 mb-6">⏱ Тайминг: {tcMin === 0 ? 'Без таймера' : `${tcMin} мин${tcInc > 0 ? ` + ${tcInc} сек` : ''}`}</p>
        <motion.button
          ...
          whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
        >
          {status === 'joining' ? t('loading') : t('invite_accept')}
        </motion.button>
        <motion.button
          onClick={() => router.push('/')}
