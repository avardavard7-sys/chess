...
export default function Chess960Page() {
  ...
  useEffect(() => {
    if (variant?.id === 'antistress' && antiTime <= 0 && !gameOver) {
      setGameOver(true);
      setGameResult('⏰ Время вышло! Вы проиграли');
      if (antiTimerRef.current) clearInterval(antiTimerRef.current);
    }
  ...
  const checkVariantWin = useCallback((chess: Chess, variantId: string, cw: number, cb: number): string | null => {
    ...
    if (variantId === 'threecheck') {
      if (cw >= 3) return 'Белые дали 3 шаха — Победа!';
      if (cb >= 3) return 'Чёрные дали 3 шаха — Поражение!';
    }
    // King of the Hill
    if (variantId === 'koth') {
      const wKing = findKing(chess, 'w');
      const bKing = findKing(chess, 'b');
      if (wKing && HILL_SQUARES.includes(wKing)) return 'Белый король на холме — Победа!';
      if (bKing && HILL_SQUARES.includes(bKing)) return 'Чёрный король на холме — Поражение!';
    }
    ...
    if (variantId === 'antichess') {
      ...
      if (whiteCount === 0) return 'Вы потеряли все фигуры — Победа!';
      if (blackCount === 0) return 'Соперник потерял все фигуры — Поражение!';
    }
    // Стандартный мат/пат
    if (chess.isCheckmate()) {
      return chess.turn() === 'b' ? 'Мат! Победа!' : 'Мат! Поражение!';
    }
    if (chess.isStalemate()) {
      if (variantId === 'antichess') {
        return chess.turn() === 'w' ? 'Пат — Победа!' : 'Пат — Поражение!';
      }
      return 'Пат — Ничья!';
    }
      ...
      if (variantId === 'atomic') {
        ...
        if (move) {
          ...
          if (move.captured) {
            ...
            setFen(newFen);
            if (kingDestroyed === 'w') { setGameOver(true); setGameResult('Ваш король взорвался — Поражение!'); }
            else if (kingDestroyed === 'b') { setGameOver(true); setGameResult('Король соперника взорвался — Победа!'); }
          } else {
            setFen(chess.fen());
          }
          setMoveHistory(prev => [...prev, move.san]);
  ...
  const handleDrop = useCallback((from: Square, to: Square): boolean => {
    ...
    if (vid === 'atomic' && move.captured) {
      ...
      setFen(newFen);
      setMoveHistory(prev => [...prev, move.san]);
      if (kingDestroyed === 'b') { setGameOver(true); setGameResult('Король соперника взорвался — Победа!'); return true; }
      if (kingDestroyed === 'w') { setGameOver(true); setGameResult('Ваш король взорвался — Поражение!'); return true; }
          ...
          {phase === 'select' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="text-center mb-8">
                <h1 className="text-3xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>Варианты шахмат</h1>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
                {VARIANTS.map(v => (
                  <motion.button key={v.id} onClick={() => startGame(v)}
                    ...
                    <p className="text-[10px] text-white/40 leading-relaxed">{v.desc}</p>
                  </motion.button>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="flex items-center justify-between mb-4">
                <motion.button onClick={() => { setPhase('select'); setGameOver(false); }}
                  className="text-white/40 hover:text-white/80 text-sm" whileHover={{ x: -3 }}>← Назад</motion.button>
              ...
              {variant.id === 'threecheck' && (
                <div className="glass p-3 rounded-xl mb-3 flex justify-around text-center">
                  <div><div className="text-[10px] text-white/30">Ваши шахи</div><div className="text-xl font-bold text-green-400">{checksWhite}/3</div></div>
                  <div><div className="text-[10px] text-white/30">Шахи соперника</div><div className="text-xl font-bold text-red-400">{checksBlack}/3</div></div>
                </div>
              )}
              {variant.id === 'koth' && (
              ...
              {variant?.id === 'antistress' && (
                <div className="mb-3 text-center">
                  <div className={`inline-flex items-center gap-3 px-6 py-3 rounded-xl ${antiTime <= 30 ? 'bg-red-500/15 border border-red-500/30' : 'glass'}`}>
                    ...
                    <span className="text-xs text-white/30">-3 сек/ход</span>
                  </div>
                </div>
              )}
                ...
                <div className="flex-1 min-w-0">
                  <div className="glass rounded-xl flex flex-col" style={{ height: Math.min(boardWidth, 300) }}>
                    ...
                    <div className="flex-1 overflow-y-auto p-2">
                      ...
                      ) : <p className="text-white/30 text-sm text-center py-8">Ходов пока нет</p>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <AnimatePresence>
                {gameOver && (
                  <motion.div className="glass p-6 rounded-2xl mt-4 text-center"
                    ...
                    <div className="text-4xl mb-3">{gameResult.includes('Победа') ? '🏆' : gameResult.includes('Ничья') ? '🤝' : '😔'}</div>
                    <div className="text-lg font-bold mb-4" style={{ fontFamily: "'Playfair Display', serif",
                      color: gameResult.includes('Победа') ? '#4ade80' : gameResult.includes('Ничья') ? '#fbbf24' : '#f87171' }}>
                      {gameResult}
                    </div>
                    <div className="flex gap-3 justify-center">
                      ...
                      <motion.button onClick={() => { setPhase('select'); setGameOver(false); }}
                        ...
                        whileHover={{ scale: 1.03 }}>Выбрать вариант</motion.button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
