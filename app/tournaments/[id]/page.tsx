...
export default function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!tournament) return <div className="min-h-screen"><Header /><div className="flex items-center justify-center min-h-screen"><div className="text-white/40">Загрузка...</div></div></div>;
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div className="glass p-6 rounded-2xl mb-6" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-center text-sm">
              <div className="p-2 rounded-xl bg-white/5"><div className="text-white/30 text-xs">Формат</div><div className="font-semibold">{tournament.format === 'knockout' ? 'Плей-офф' : 'Швейцарская'}</div></div>
              <div className="p-2 rounded-xl bg-white/5"><div className="text-white/30 text-xs">Контроль</div><div className="font-semibold">{tournament.time_control}</div></div>
              <div className="p-2 rounded-xl bg-white/5"><div className="text-white/30 text-xs">Участники</div><div className="font-semibold text-yellow-400">{participants.length}/{tournament.max_participants}</div></div>
              <div className="p-2 rounded-xl bg-white/5"><div className="text-white/30 text-xs">Раунд</div><div className="font-semibold">{tournament.current_round}/{tournament.total_rounds}</div></div>
              <div className="p-2 rounded-xl bg-white/5"><div className="text-white/30 text-xs">Длительность</div><div className="font-semibold">{tournament.duration_hours}ч</div></div>
            </div>
            ...
            {countdown && tournament.status === 'upcoming' && (
              <motion.div className="mt-4 p-4 rounded-xl text-center" style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)' }}
                ...
                <div className="text-xs text-white/40 uppercase tracking-wider mb-1">До начала турнира</div>
          ...
          {matches.length > 0 && (
            <div className="glass p-4 rounded-2xl mb-6">
              <h3 className="text-sm font-bold text-white/40 uppercase tracking-wider mb-4">Турнирная сетка</h3>
          ...
          <div className="glass rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-white/10 flex items-center justify-between">
              <span className="text-sm font-bold text-white/60 uppercase tracking-wider">Таблица турнира ({participants.length})</span>
              {participants.length > 0 && (
                <span className="text-xs text-white/20">W — победы | L — поражения | D — ничьи</span>
              )}
            </div>
            {participants.length === 0 ? (
              <div className="p-8 text-center text-white/30">Пока нет участников</div>
            ) : (
