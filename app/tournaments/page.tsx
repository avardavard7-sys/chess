...
export default function TournamentsPage() {
  ...
  const handleJoin = async (tid: string) => {
    // Блокировка от двойного клика
    if (myTournaments.has(tid)) return;
    if ((handleJoin as any)._joining) return;
    (handleJoin as any)._joining = true;
    ...
    try {
      ...
      if (error) {
        // Дубликат ключа — просто обновляем локальное состояние
        if (error.code === '23505') {
          setMyTournaments(new Set([...myTournaments, tid]));
          return;
        }
      ...
      (handleJoin as any)._joining = false;
    }
  };
      ...
      <main className="pt-24 pb-12 px-4">
        <div className="max-w-4xl mx-auto">
          <motion.div className="text-center mb-8" initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-white/50">{t('compete_students')}</p>
          </motion.div>
          ...
          {loading ? (
            ...
            <div className="glass p-10 rounded-2xl text-center">
              ...
              <p className="text-white/40">{t('tournaments_title')}</p>
              <p className="text-white/20 text-sm mt-2">{t('admin_soon')}</p>
            </div>
          ) : (
