...
function GameContent({ rawMode }: { rawMode: string }) {
  ...
  const tournamentMatchId = searchParams.get('tournament_match') || undefined;
  ...
  // Тайминг из URL (friend invite)
  const tcMin = searchParams.get('tc_min');
  const tcInc = searchParams.get('tc_inc');
  const isFriendMode = gameMode === 'friend' || searchParams.get('mode') === 'friend';
  const urlTimeControl = tcMin !== null ? { minutes: parseInt(tcMin), increment: parseInt(tcInc || '0') } : isFriendMode ? { minutes: 5, increment: 0 } : null;

  const [timeControl, setTimeControl] = useState<{ minutes: number; increment: number } | null>(
    isKids ? { minutes: 0, increment: 0 } : urlTimeControl
  );
  const [showTimeModal, setShowTimeModal] = useState(!isKids && !resumeGameId && !tournamentMatchId && !urlTimeControl && !isFriendMode);
  ...
  const handlePlayAgain = () => {
    if (gameMode === 'friend' && sessionId && rematchChannelRef.current) {
      ...
      setGameOverData(null);
      setGameKey((k) => k + 1);
      if (!isKids) setShowTimeModal(true);
    }
  };
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <main className="relative z-10 pt-24 pb-12 px-4">
        ...
        {showTimeModal && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            <TimeControlSelector
              title={`${diffInfo.icon} ${diffInfo.label} — выберите время`}
              onSelect={(time, minutes, increment) => { setTimeControl({ minutes, increment }); setShowTimeModal(false); }}
            />
          </motion.div>
        )}
        ...
        >
          <ChessBoard
            ...
            onGameOver={(result, eloChange, coinChange, gameId, reason) => setGameOverData({ result, eloChange, coinChange, gameId, reason })}
          />
        </motion.div>
        )}
      </main>
      ...
      <GameOverModal
        ...
        coinChange={gameOverData?.coinChange ?? null}
        gameId={gameOverData?.gameId}
        reason={gameOverData?.reason}
        onPlayAgain={tournamentId ? () => router.push(`/tournaments/${tournamentId}`) : handlePlayAgain}
      />
      ...
      {gameOverData && tournamentId && (
        <motion.div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50"
          ...
          <motion.button onClick={() => router.push(`/tournaments/${tournamentId}`)}
            className="px-6 py-3 rounded-xl text-sm font-bold text-black shadow-lg"
        ...
        {rematchRequest && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center"
            ...
            <motion.div className="glass p-8 rounded-2xl text-center max-w-sm mx-4"
              ...
              <h3 className="text-xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>{t('rematch_title')}</h3>
              <p className="text-white/50 text-sm mb-6">{t("rematch_desc")}</p>
