...
function GameContent({ rawMode }: { rawMode: string }) {
  ...
  const [timeControl, setTimeControl] = useState<{ minutes: number; increment: number } | null>(
    isKids ? { minutes: 0, increment: 0 } : null
  );
  const [showTimeModal, setShowTimeModal] = useState(!isKids && !resumeGameId && !tournamentMatchId);
      ...
      <main className="relative z-10 pt-24 pb-12 px-4">
        ...
        >
          <ChessBoard
            ...
            onGameOver={(result, eloChange, coinChange) => setGameOverData({ result, eloChange, coinChange })}
          />
        </motion.div>
        )}
      </main>
