...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadProfile = async (uid: string) => {
      let { data: prof } = await getProfile(uid);
      if (!prof) {
        const { data: { session } } = await supabase.auth.getSession();
        const meta = session?.user?.user_metadata || {};
        const username = meta.name || meta.full_name || session?.user?.email?.split('@')[0] || 'Player';
        const avatar_url = meta.avatar_url || meta.picture || '';
        await supabase.from('profiles').upsert({
          id: uid, username, avatar_url, elo_rating: 0,
          rank: '\u{1F476} \u041c\u0430\u043b\u044b\u0448',
          games_played: 0, games_won: 0, games_lost: 0, games_draw: 0,
        }, { onConflict: 'id' });
        const { data: newProf } = await getProfile(uid);
        prof = newProf;
      }
      if (prof) setProfile(prof);
      const { data: history } = await getGameHistory(uid, 10);
      ...
      <div className="min-h-screen">
        ...
        <div className="flex items-center justify-center min-h-screen pt-24 px-4">
          <motion.div className="glass p-10 rounded-2xl text-center max-w-md w-full" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h2 className="text-3xl font-bold mb-3" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>{t('profile_title')}</h2>
            <p className="text-white/60 mb-8">{t('sign_in_profile')}</p>
            <motion.button
      ...
      <main className="pt-24 pb-12 px-4">
        <div className="max-w-4xl mx-auto">
          <motion.div className="flex items-center justify-between mb-8" initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-3xl font-bold" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>{t('my_profile')}</h1>
          ...
          {profile && <ProfileCard profile={profile} gameHistory={gameHistory} onProfileUpdate={(updated) => setProfile(updated as typeof profile)} />}
          {!profile && (
            <div className="glass p-8 rounded-2xl text-center">
              <p className="text-white/50">{t('sign_in_required')}</p>
            </div>
          )}
        </div>
      </main>
