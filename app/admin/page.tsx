  ...
  useEffect(() => {
    const check = async () => {
      ...
      if (ok) {
        ...
        const { data } = await supabase.from('profiles').select('id, username, elo_rating, puzzle_rating, is_admin, is_trainer, games_played, coins').order('elo_rating', { ascending: false });
        setUsers(data || []);
        ...
        const { data: st } = await supabase.from('school_students').select('*').order('rating', { ascending: false });
      ...
      <main className="pt-24 pb-12 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            {([['dashboard', 'Дашборд', '📊'], ['live', 'Live игры', '🔴'], ['tournaments', 'Турниры', '🏆'], ['create', 'Создать турнир', '➕'], ['users', 'Пользователи', '👥'], ['coins', 'Коины', '🪙'], ['rating', 'Рейтинг', '📈'], ['trainers', 'Тренеры', '👨‍🏫'], ['shop', 'Магазин', '🛍️'], ['orders', 'Заказы', '📩'], ['analytics', 'Аналитика', '📉']] as [Tab, string, string][]).map(([id, label, icon]) => (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="text-left text-white/30 text-xs">
                    ...
                    <th className="px-4 py-2">Puzzle ELO</th><th className="px-4 py-2">Коины</th><th className="px-4 py-2">Партий</th><th className="px-4 py-2">Админ</th><th className="px-4 py-2">Тренер</th><th className="px-4 py-2">Действия</th>
                  </tr></thead>
                  <tbody>
                    {users.map((u, i) => (
                      <tr key={u.id} className="border-t border-white/5 hover:bg-white/5">
                        ...
                        <td className="px-4 py-2">
                          <div className="flex items-center gap-1">
                            <button onClick={async () => {
                              ...
                              const { data } = await supabase.from('profiles').select('id, username, elo_rating, puzzle_rating, is_admin, is_trainer, games_played, coins').order('elo_rating', { ascending: false }); setUsers(data || []);
                            }}
                        ...
                        <td className="px-4 py-2">
                          <button onClick={async () => { if (confirm(`Удалить ${u.username}?`)) { await supabase.from('profiles').delete().eq('id', u.id); const { data } = await supabase.from('profiles').select('id, username, elo_rating, puzzle_rating, is_admin, is_trainer, games_played, coins').order('elo_rating', { ascending: false }); setUsers(data || []); setMsg('Пользователь удалён'); } }}
          ...
          {tab === 'rating' && (
            <div className="space-y-6">
              ...
              <div className="glass p-6 rounded-2xl">
                <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>Тренеры</h3>
                <div className="flex gap-2 mb-4">
                  ...
                  <motion.button onClick={async () => {
                    ...
                    whileTap={{ scale: 0.95 }}>Добавить</motion.button>
                </div>
                <div className="space-y-2">
                  {schoolTrainers.map(t => {
                    const trStudents = schoolStudents.filter(s => s.trainer_id === t.id);
                    return (
                      <div key={t.id} className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/5">
                        ...
                        <button onClick={async () => {
                          ...
                          await supabase.from('school_trainers').delete().eq('id', t.id);
                          const { data } = await supabase.from('school_trainers').select('*').order('name');
                          setSchoolTrainers(data || []);
              ...
              {/* Добавить ученика */}
              <div className="glass p-6 rounded-2xl">
                <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>Добавить ученика</h3>
                
                {/* Выбрать из зарегистрированных */}
                <div className="mb-3">
                  <div className="text-xs text-white/40 mb-1">Выбрать из зарегистрированных пользователей</div>
                  <select onChange={e => {
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  ...
                  <input value={studentForm.rating} onChange={e => setStudentForm({ ...studentForm, rating: e.target.value })} placeholder="Баллы" type="number"
                    className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder-white/30" />
                  <select value={studentForm.trainer_id} onChange={e => setStudentForm({ ...studentForm, trainer_id: e.target.value })}
                    className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-sm">
                    <option value="">Выберите тренера</option>
                    {schoolTrainers.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                  </select>
                </div>
                ...
                <motion.button onClick={async () => {
                  ...
                  await supabase.from('school_students').insert({
                    ...
                    rating,
                    trainer_id: studentForm.trainer_id || null,
                  ...
                  // Если выбран из списка — обновляем trainer_id в профиле
                  if (profileId && studentForm.trainer_id) {
                    await supabase.from('profiles').update({ trainer_id: studentForm.trainer_id }).eq('id', profileId);
                  }
                  setStudentForm({ full_name: '', birth_year: '', rating: '0', trainer_id: '', achievements: '', profile_id: '' });
                  const { data } = await supabase.from('school_students').select('*').order('rating', { ascending: false });
                  setSchoolStudents(data || []);
                  setMsg('Ученик добавлен!');
                }} className="w-full py-2.5 rounded-xl text-sm font-semibold text-black"
                  style={{ background: 'linear-gradient(135deg, #f59e0b, #d97706)' }}
                  whileTap={{ scale: 0.97 }}>Добавить ученика</motion.button>
              </div>

              {/* Список учеников */}
              <div className="glass rounded-2xl overflow-hidden">
                <div className="p-4 border-b border-white/10"><span className="text-xs text-white/40 uppercase tracking-wider">Ученики ({schoolStudents.length})</span></div>
                <div className="space-y-2 p-3">
                  {schoolStudents.map(s => {
                    const RANKS = [{ min: 0, name: 'Без разряда', short: 'Без', color: '#6b7280' }, { min: 300, name: '5 разряд', short: '5р', color: '#CD7F32' }, { min: 500, name: '4 разряд', short: '4р', color: '#3b82f6' }, { min: 1000, name: '3 разряд', short: '3р', color: '#8b5cf6' }, { min: 1150, name: '2 разряд', short: '2р', color: '#f59e0b' }, { min: 1300, name: '1 разряд', short: '1р', color: '#ef4444' }];
                      ...
                      <div key={s.id} className={`rounded-xl bg-white/5 p-4 ${s.graduated ? 'opacity-50' : ''}`}>
                        ...
                        <div className="flex items-center gap-3 mb-3">
                          <div className="flex-1">
                            ...
                            {s.birth_year && <span className="text-xs text-white/20 ml-2">({s.birth_year} г.р.)</span>}
                            <div className="text-xs text-white/30">{schoolTrainers.find(t => t.id === s.trainer_id)?.name || 'Нет тренера'}</div>
                          </div>
                        ...
                        {nextRank && (
                          <div className="mb-3">
                            <div className="flex justify-between text-[10px] text-white/20 mb-1">
                              <span>{rank.short} ({rank.min})</span>
                              <span>{nextRank.short} ({nextRank.min})</span>
                            </div>
                            ...
                            <div className="text-[10px] text-white/15 text-center mt-0.5">{progress}% до {nextRank.short}</div>
                          </div>
                        )}
                        ...
                        <div className="flex items-center gap-1 flex-wrap">
                          ...
                          {s.rating >= 1300 && !s.graduated && (
                            ...
                            }} className="px-2 py-0.5 rounded text-[10px] bg-purple-500/10 text-purple-400 hover:bg-purple-500/20">Перевести</button>
                          )}
                          <button onClick={async () => {
                            ...
                            const { data } = await supabase.from('school_students').select('*').order('rating', { ascending: false });
                            setSchoolStudents(data || []);
                          }} className="px-2 py-0.5 rounded text-[10px] bg-red-500/10 text-red-400 hover:bg-red-500/20">Удалить</button>
                        </div>
          ...
          {tab === 'trainers' && (
            <div className="space-y-4">
              <div className="glass p-6 rounded-2xl">
                <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>Назначенные тренеры</h3>
                <p className="text-xs text-white/30 mb-4">Назначайте тренеров во вкладке "Пользователи" → кнопка "Назначить". Здесь можно убрать доступ.</p>
                {users.filter(u => u.is_trainer).length === 0 ? (
                  <div className="text-center py-8">
                    <div className="text-4xl mb-3">👨‍🏫</div>
                    <p className="text-white/40">Нет назначенных тренеров</p>
                    <p className="text-white/20 text-xs mt-1">Перейдите в "Пользователи" и нажмите "Назначить" рядом с именем</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {users.filter(u => u.is_trainer).map(u => (
                      <div key={u.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                        ...
                        <span className="text-xs text-green-400 px-2 py-1 rounded-full bg-green-500/10">Тренер ✓</span>
                        <button onClick={async () => {
          ...
          {tab === 'shop' && (
            <div className="space-y-4">
              <div className="glass p-6 rounded-2xl">
                <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>Добавить товар</h3>
                <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                {/* Загрузка фото */}
                <div className="mb-3">
                  <label className="block text-xs text-white/40 mb-2">Фото товара (можно несколько)</label>
                  ...
                  {uploading && <p className="text-xs text-yellow-400 mt-1 animate-pulse">Загрузка фото...</p>}
                </div>
              ...
              {/* Product list */}
              <div className="glass rounded-2xl overflow-hidden">
                <div className="p-4 border-b border-white/10"><span className="text-xs text-white/40 uppercase tracking-wider">Товары ({shopProducts.length})</span></div>
                <div className="space-y-2 p-3">
                  {shopProducts.map(p => (
                    <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                      ...
                      <div className="flex gap-1 flex-shrink-0">
                        ...
                        <button onClick={async () => {
                          ...
                          const { data } = await supabase.from('shop_products').select('*').order('created_at', { ascending: false });
                          setShopProducts(data || []);
                        }} className="px-2 py-1 rounded text-[10px] bg-red-500/10 text-red-400">Удалить</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {tab === 'orders' && (
            <div className="space-y-4">
              <div className="glass p-4 rounded-xl">
                ...
                {shopOrders.length === 0 ? (
                  <div className="text-center py-8"><div className="text-4xl mb-3">📩</div><p className="text-white/40">Заказов пока нет</p></div>
                ) : (
                  <div className="space-y-2">
                    {shopOrders.map(o => (
          ...
          {tab === 'analytics' && (
            <div className="space-y-4">
              ...
              <div className="glass p-4 rounded-xl flex flex-wrap items-center gap-3">
                <span className="text-sm font-bold">Период:</span>
              ...
              {/* Trainer filter */}
              <div className="glass p-4 rounded-xl flex flex-wrap items-center gap-3">
                <span className="text-sm font-bold">Тренер:</span>
                ...
                {users.filter(u => u.is_trainer).map(t => (
                  <button key={t.id} onClick={() => setAnalyticsTrainer(t.id)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${analyticsTrainer === t.id ? 'text-black bg-green-400' : 'text-white/50 border border-white/10'}`}>
                    {t.username}
                  </button>
                ))}
              </div>
                  ...
                  <>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div className="glass p-4 rounded-xl text-center">
                        ...
                        <div className="text-xs text-white/40">Заработано {periodLabel}</div>
                      </div>
                      <div className="glass p-4 rounded-xl text-center">
                        <div className="text-2xl font-bold text-green-400" style={{ fontFamily: "'Playfair Display', serif" }}>{fromGames}</div>
                        <div className="text-xs text-white/40">За игры</div>
                      </div>
                      <div className="glass p-4 rounded-xl text-center">
                        <div className="text-2xl font-bold text-purple-400" style={{ fontFamily: "'Playfair Display', serif" }}>{fromTrainer}</div>
                        <div className="text-xs text-white/40">От тренеров</div>
                      </div>
                      <div className="glass p-4 rounded-xl text-center">
                        <div className="text-2xl font-bold text-blue-400" style={{ fontFamily: "'Playfair Display', serif" }}>{fromAdmin + fromTournament}</div>
                        <div className="text-xs text-white/40">Админ + Турниры</div>
                      </div>
                    </div>

                    {/* Top earners */}
                    <div className="glass p-4 rounded-xl">
                      <div className="text-sm font-bold mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>Топ учеников {periodLabel}</div>
                      {topUsers.length === 0 ? (
                        <p className="text-center text-white/30 py-4">Нет данных за этот период</p>
                      ) : topUsers.map((u, i) => (
                        ...
                        <div className="glass p-4 rounded-xl border border-green-500/20">
                          ...
                          <div className="grid grid-cols-3 gap-3 mb-3">
                            <div className="p-3 rounded-lg bg-white/5 text-center">
                              ...
                              <div className="text-[10px] text-white/30">Коинов начислено</div>
                            </div>
                            <div className="p-3 rounded-lg bg-white/5 text-center">
                              <div className="text-xl font-bold text-purple-400">{trainerOps}</div>
                              <div className="text-[10px] text-white/30">Операций</div>
                            </div>
                            <div className="p-3 rounded-lg bg-white/5 text-center">
                              <div className="text-xl font-bold text-blue-400">{trainerStudents}</div>
                              <div className="text-[10px] text-white/30">Ученикам</div>
                            </div>
                          </div>
                        </div>
                      );
                    ...
                    {/* Recent transactions */}
                    <div className="glass p-4 rounded-xl">
                      <div className="text-sm font-bold mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>Последние операции</div>
                      <div className="max-h-60 overflow-y-auto space-y-1">
                        {filtered.slice(0, 30).map(t => {
                          const u = users.find(u => u.id === t.user_id);
                          return (
                            <div key={t.id} className="flex items-center gap-2 text-xs px-2 py-1 rounded hover:bg-white/5">
                              ...
                              <span className="text-white/20">{t.source}</span>
                              <span className="text-white/15">{new Date(t.created_at).toLocaleDateString('ru-RU')}</span>
                            </div>
                          );
                        })}
                        {filtered.length === 0 && <p className="text-center text-white/20 py-4">Нет операций</p>}
                      </div>
                    </div>
                  </>
                );
