...
export default function WatchGamePage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  useEffect(() => {
    const u = () => setBoardWidth(Math.min(window.innerWidth < 640 ? window.innerWidth - 24 : window.innerWidth < 1024 ? 500 : 600, 640));
  ...
  if (!game) return (
    <div className="min-h-screen"><Header />
      <div className="flex items-center justify-center min-h-screen"><div className="text-white/40">{t('loading')}</div></div>
    </div>
  );
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-4">
              <div className="glass p-4 rounded-xl">
                <div className="text-xs text-white/40 uppercase tracking-wider mb-2">{t('info_label')}</div>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-white/40">{t('mode_label')}</span><span className="text-white/70">{game.mode}</span></div>
                  <div className="flex justify-between"><span className="text-white/40">{t('status_label')}</span><span className={isFinished ? 'text-white/40' : 'text-green-400'}>{isFinished ? 'Завершена' : 'Идёт'}</span></div>
                  <div className="flex justify-between"><span className="text-white/40">{t('moves_count_label')}</span><span className="text-white/70">{moves.length}</span></div>
                  {game.tournament_id && <div className="flex justify-between"><span className="text-white/40">{t('menu_tournaments')}</span><span className="text-yellow-400">Да</span></div>}
                </div>
              </div>
              ...
              <div className="glass rounded-xl overflow-hidden flex-1">
                <div className="p-3 border-b border-white/10">
                  <span className="text-xs text-white/40 uppercase tracking-wider">{t('moves_label')}</span>
                </div>
                <div className="overflow-y-auto p-3 font-mono text-sm" style={{ maxHeight: boardWidth - 100 }}>
                  {moves.length === 0 ? (
                    <p className="text-white/20 text-center py-4">{t('waiting_moves')}</p>
                  ) : (
