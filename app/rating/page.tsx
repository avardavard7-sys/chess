...
const RANKS = [
  ...
  { min: 300, name: '5 разряд', short: '5р', color: '#CD7F32' },
  { min: 500, name: '4 разряд', short: '4р', color: '#3b82f6' },
  { min: 1000, name: '3 разряд', short: '3р', color: '#8b5cf6' },
  { min: 1150, name: '2 разряд', short: '2р', color: '#f59e0b' },
  { min: 1300, name: '1 разряд', short: '1р', color: '#ef4444' },
];
      ...
      <main className="pt-24 pb-12 px-4">
        <div className="max-w-3xl mx-auto">
          <motion.div className="text-center mb-8" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-3xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>Внутренний рейтинг</h1>
            <p className="text-white/40">Шахматная школа «Ход Конём»</p>
          </motion.div>

          <div className="glass p-3 rounded-xl mb-6">
            <input type="text" placeholder="Поиск по ФИО ученика..." value={search}
          ...
          <AnimatePresence>
            {selected && (
              <motion.div className="glass p-6 rounded-2xl mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                ...
                {selected.trainer_id && (
                  <div className="text-sm text-white/50 mb-4">Тренер: <span className="text-yellow-400">{getTrainerName(selected.trainer_id)}</span></div>
                )}

                {/* Баллы + Разряд + Посещаемость */}
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div className="glass p-3 rounded-xl text-center">
                    <div className="text-xs text-white/40 mb-1">Баллы</div>
                    <div className="text-2xl font-bold text-yellow-400" style={{ fontFamily: "'Playfair Display', serif" }}>{selected.rating}</div>
                  </div>
                  <div className="glass p-3 rounded-xl text-center">
                    <div className="text-xs text-white/40 mb-1">Разряд</div>
                  ...
                  <div className="glass p-3 rounded-xl text-center">
                    <div className="text-xs text-white/40 mb-1">Посещаемость</div>
                    {(() => {
                ...
                {getStudentAwards(selected.id).length > 0 && (
                  <div className="mb-4">
                    <div className="text-xs text-white/40 mb-2">Награды</div>
                ...
                {selected.achievements && selected.achievements.length > 0 && (
                  <div className="mb-4">
                    <div className="text-xs text-white/40 mb-2">Достижения</div>
                    <div className="flex flex-wrap gap-2">
                ...
                {getStudentHomework(selected.id).length > 0 && (
                  <div className="mb-4">
                    <div className="text-xs text-white/40 mb-2">Домашние задания</div>
                    <div className="space-y-1">
                      {getStudentHomework(selected.id).map((h: any) => (
                        <div key={h.id} className="flex items-center gap-2 text-xs px-2 py-1.5 rounded-lg bg-white/5">
                          ...
                          {h.completed ? <span className="text-green-400">{h.result_percent}% ✓</span> : <span className="text-yellow-400">В процессе</span>}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {getStudentNotes(selected.id).length > 0 && (
                  <div className="mb-4">
                    <div className="text-xs text-white/40 mb-2">Комментарии тренера</div>
                    <div className="space-y-1">
                ...
                {selected.trainer_id && getTrainerSchedule(selected.trainer_id).length > 0 && (
                  <div className="mb-4">
                    <div className="text-xs text-white/40 mb-2">Расписание занятий</div>
                    <div className="flex flex-wrap gap-2">
                ...
                {(() => {
                  ...
                  return (
                    <div className="mb-4">
                      <div className="text-xs text-white/40 mb-2">Журнал посещаемости ({present}/{records.length})</div>
                ...
                {selected.rating_history && selected.rating_history.length >= 2 && (
                  <div>
                    <div className="text-xs text-white/40 mb-2">График рейтинга</div>
          ...
          <div className="glass rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-white/10">
              <span className="text-xs text-white/40 uppercase tracking-wider">Все ученики ({filtered.length})</span>
            </div>
            {filtered.length === 0 ? (
