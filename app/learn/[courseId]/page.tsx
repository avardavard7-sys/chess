...
export default function CoursePage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  const onPieceDrop = useCallback((s: Square, t: Square) => state === 'thinking' ? checkPlayerMove(s, t) : false, [checkPlayerMove, state]);

  const handleSquareClick = useCallback((sq: Square) => {
    if (state !== 'thinking') return;
    const chess = chessRef.current;
    const piece = chess.get(sq);
    if (selectedSquare) { const ok = checkPlayerMove(selectedSquare, sq); setSelectedSquare(null); if (!ok && piece && piece.color === chess.turn()) setSelectedSquare(sq); return; }
    if (piece && piece.color === chess.turn()) setSelectedSquare(sq);
  }, [state, selectedSquare, checkPlayerMove]);
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-shrink-0">
                  ...
                  <motion.div className={shaking ? 'shake' : ''}>
                    <Chessboard position={fen} onSquareClick={handleSquareClick} onPieceDrop={onPieceDrop} customSquareStyles={sqStyles} customArrowColor="rgba(74,222,128,0.8)" customArrows={customArrows} boardWidth={boardWidth} customLightSquareStyle={{ backgroundColor: '#f0d9b5' }} customDarkSquareStyle={{ backgroundColor: '#b58863' }} boardOrientation={playerColor} animationDuration={200} arePiecesDraggable={state === 'thinking'} />
                  </motion.div>
