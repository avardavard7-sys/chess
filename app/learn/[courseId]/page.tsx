...
export default function CoursePage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  useEffect(() => {
    const u = () => { const w = window.innerWidth; setBoardWidth(Math.min(w < 640 ? w - 24 : w < 1024 ? 480 : 580, 620)); };
  ...
  const checkPlayerMove = useCallback((from: string, to: string, promotion?: string) => {
    ...
    if (pUci === exp) {
      ...
      return true;
    } else { setState('wrong'); triggerShake(); playSound('wrong'); setFen(chessRef.current.fen()); setTimeout(() => setState('thinking'), 1200); return false; }
  ...
  const showHint = () => {
    ...
    // 2-ступенчатая подсказка:
    // 1-е нажатие — только подсвечиваем фигуру
    // 2-е нажатие — показываем стрелку лучшего хода
    if (!showHintArrow) {
      // Первое нажатие — подсветка фигуры
      setCustomArrows([]);
      setShowHintArrow(true);
    } else {
      // Второе нажатие — полная стрелка хода
      ...
      <div className="min-h-screen">
        ...
        <div className="flex items-center justify-center min-h-screen pt-24">
          <div className="glass p-10 rounded-2xl text-center">
            <p className="text-white/60">{t('course_not_found')}</p>
            <motion.button onClick={() => router.push('/learn')} className="mt-4 px-6 py-2 rounded-xl text-yellow-400 border border-yellow-400/30" whileHover={{ scale: 1.02 }}>{t('to_courses')}</motion.button>
          </div>
        </div>
      </div>
    );
  ...
  // Подсветка фигуры-подсказки (1-е нажатие кнопки подсказка)
  if (showHintArrow && currentPuzzle && currentPuzzle.moves[moveStep]) {
    const hintFrom = currentPuzzle.moves[moveStep].slice(0, 2);
    sqStyles[hintFrom] = {
      ...sqStyles[hintFrom],
      background: 'radial-gradient(circle, rgba(250,204,21,0.75) 30%, rgba(250,204,21,0.25) 70%)',
        ...
        <div className="max-w-5xl mx-auto">
          ...
          {state === 'loading' && (
            <motion.div className="glass p-10 rounded-2xl text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <h3 className="text-lg font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>{t('loading_puzzles')}</h3>
              <p className="text-white/40 text-sm">{t('loading_puzzles')}</p>
          ...
          {state === 'error' && (
            <motion.div className="glass p-10 rounded-2xl text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <h3 className="text-lg font-bold mb-2">{t('load_failed')}</h3>
              <div className="flex gap-3 justify-center mt-4">
                ...
                <motion.button onClick={() => router.push('/learn')} className="px-6 py-3 rounded-xl border border-white/15 text-white/60" whileHover={{ scale: 1.03 }}>{t('to_courses')}</motion.button>
              </div>
            </motion.div>
          )}
          ...
          {state === 'complete' && (
            <motion.div className="glass p-10 rounded-2xl text-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              ...
              <h2 className="text-2xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif", color: '#4ade80' }}>{t('course_complete')}</h2>
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-shrink-0">
                  <AnimatePresence mode="wait">
                    {state === 'correct' && (
                      <motion.div key="c" className="flex items-center gap-2 mb-3 px-4 py-2.5 rounded-xl" style={{ background: 'rgba(74,222,128,0.15)', border: '1px solid rgba(74,222,128,0.3)' }} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                        <span className="text-sm font-bold text-green-400">{t('correct_answer')}</span>
                    ...
                    {state === 'wrong' && (
                      <motion.div key="w" className="flex items-center gap-2 mb-3 px-4 py-2.5 rounded-xl" style={{ background: 'rgba(248,113,113,0.15)', border: '1px solid rgba(248,113,113,0.3)' }} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                        <span className="text-sm font-bold text-red-400">{t('learn_wrong')}</span>
                      </motion.div>
                    )}
                    {state === 'thinking' && (
                  ...
                  <motion.div className={shaking ? 'shake' : ''}>
                    <Chessboard position={fen} onSquareClick={handleSquareClick} onPieceDrop={onPieceDrop} onPromotionPieceSelect={onPromotion} customSquareStyles={sqStyles} customArrowColor="rgba(74,222,128,0.8)" customArrows={customArrows} boardWidth={boardWidth} customLightSquareStyle={{ backgroundColor: boardTheme.light, ...boardTheme.lightStyle }} customDarkSquareStyle={{ backgroundColor: boardTheme.dark, ...boardTheme.darkStyle }} boardOrientation={playerColor} animationDuration={200} arePiecesDraggable={state === 'thinking'} />
                  </motion.div>

                  <div className="flex gap-2 mt-3">
                    <motion.button onClick={showHint} disabled={state !== 'thinking' || customArrows.length > 0} className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/10 disabled:opacity-30" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>{t('learn_hint')}</motion.button>
                    <motion.button onClick={retryPuzzle} className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-white/15 text-white/50 hover:bg-white/5" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>{t('learn_retry')}</motion.button>
                    <motion.button onClick={nextPuzzle} disabled={puzzleIndex >= total - 1 && state !== 'correct'} className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-white/15 text-white/50 hover:bg-white/5 disabled:opacity-30" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>{t('learn_skip')}</motion.button>
                  </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-4">
                  <div className="glass p-4 rounded-xl">
                    <div className="text-xs text-white/40 uppercase tracking-wider mb-2">{t('learn_hint')}</div>
                    <p className="text-sm text-white/70">
                      {customArrows.length > 0
                        ? '➡️ Стрелка показывает лучший ход!'
                        : showHintArrow
                        ? '💡 Подсвечена фигура для хода. Нажмите ещё раз чтобы увидеть точный ход'
                        : 'Нажмите "Подсказка" чтобы увидеть'}
                    </p>
                  </div>

                  <div className="glass p-4 rounded-xl">
                    <div className="text-xs text-white/40 uppercase tracking-wider mb-2">{t('puzzle_rating_label')}</div>
                    <p className="text-sm text-white/70">{t('puzzle_difficulty')}: {currentPuzzle.rating}</p>
                  </div>

                  <div className="glass rounded-xl overflow-hidden flex-1">
                    <div className="p-3 border-b border-white/10">
                      <span className="text-xs text-white/40 uppercase tracking-wider">{t('course_puzzles')}</span>
                    </div>
