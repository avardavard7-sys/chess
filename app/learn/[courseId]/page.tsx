...
export default function CoursePage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  const retryPuzzle = () => {
    ...
    if (currentPuzzle.moves.length > 0) {
      const u = currentPuzzle.moves[0];
      timeoutRef.current = setTimeout(() => { try { chess.move({ from: u.slice(0, 2), to: u.slice(2, 4), promotion: u.length === 5 ? u[4] as 'q' | 'r' | 'b' | 'n' : undefined }); setFen(chess.fen()); setMoveStep(1); } catch {  } }, 400);
    }
  };
