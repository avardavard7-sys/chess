...
export default function ShopPage() {
  ...
  const buyProduct = async (product: any) => {
    ...
    if (userCoins < product.price) { setBuyMsg(t('shop_not_enough')); return; }
    setBuying(true);
    // 1. Списываем коины
      ...
      <main className="pt-24 pb-12 px-4">
        <div className="max-w-5xl mx-auto">
          <motion.div className="text-center mb-8" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-3xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>🛍️ {t('shop_title')}</h1>
            <p className="text-white/40">{t('menu_shop_coins')}</p>
            <motion.div className="inline-flex items-center gap-2 mt-3 px-4 py-2 rounded-full glass"
              ...
              <span className="text-xs text-white/30">{t('coins_label')}</span>
            </motion.div>
          </motion.div>
          ...
          {products.length === 0 ? (
            <div className="glass p-10 rounded-2xl text-center">
              ...
              <p className="text-white/40">{t('shop_title')}</p>
              <p className="text-white/20 text-xs mt-1">{t('loading')}</p>
            </div>
          ) : (
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white/30">{t('coins_label')}</span>
                      </div>
                    ...
                    {!userId && <p className="text-xs text-white/20 text-center mt-2">{t('sign_in_to_buy')}</p>}
                  </div>
                </motion.div>
              </motion.div>
            )}
