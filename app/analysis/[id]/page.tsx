...
export default function AnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setBoardWidth(Math.min(w < 640 ? w - 64 : w < 1024 ? 360 : 420, 460));
    };
  ...
  const startAnalysis = useCallback(async () => {
    ...
    setAnalyzing(true);
    try {
      const result = await analyzeGame(game.moves_json, (c, t) => setProgress({ current: c, total: t }), 10);
  ...
  if (!game) return <div className="min-h-screen"><Header /><div className="flex items-center justify-center min-h-screen pt-24"><div className="glass p-10 rounded-2xl text-center"><div className="text-5xl mb-4">❌</div><p className="text-white/60">Партия не найдена</p><motion.button onClick={() => router.push('/analysis')} className="mt-4 px-6 py-2 rounded-xl text-yellow-400 border border-yellow-400/30" whileHover={{ scale: 1.02 }}>Назад</motion.button></div></div></div>;
      ...
      <main className="pt-24 pb-12 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div className="flex items-center gap-3 mb-6" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-xl font-bold" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>Анализ партии</h1>
          ...
          <AnimatePresence>
            {analyzing && (
              <motion.div className="glass p-6 rounded-2xl mb-6 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <h3 className="text-lg font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>Анализируем партию...</h3>
                <p className="text-white/50 text-sm mb-4">Ход {progress.current} из {progress.total}</p>
          ...
          {!analysis && !analyzing && (
            <motion.div className="glass p-8 rounded-2xl mb-6 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <h3 className="text-xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>Запустить анализ</h3>
              <p className="text-white/50 text-sm mb-6">Stockfish проанализирует каждый ход</p>
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-6">
                <div className="glass p-3 rounded-xl text-center">
                  <div className="text-xs text-white/40 mb-1">♔ Белые</div>
                ...
                <div className="glass p-3 rounded-xl text-center">
                  <div className="text-xs text-white/40 mb-1">♚ Чёрные</div>
                ...
                <div className="glass p-3 rounded-xl text-center">
                  <div className="text-xs text-white/40 mb-1">Дебют</div>
                  <div className="text-xs font-semibold text-white/80 leading-tight">{analysis.opening}</div>
                </div>
                <div className="glass p-3 rounded-xl text-center">
                  <div className="text-xs text-white/40 mb-1">Результат</div>
                  <div className="text-xl">{game.result === 'win' ? '🏆' : game.result === 'loss' ? '😔' : '🤝'}</div>
                </div>
                {/* Performance Rating */}
                <div className="glass p-3 rounded-xl text-center">
                  <div className="text-xs text-white/40 mb-1">Перформанс</div>
                ...
                {phases && (
                  <div className="glass p-3 rounded-xl text-center">
                    <div className="text-xs text-white/40 mb-1">Фазы игры</div>
                    ...
                    <div className="flex justify-center gap-1 mt-1 text-[9px] text-white/20">
                      <span>Деб</span><span>Мит</span><span>Энд</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {(['white', 'black'] as const).map(color => (
                  <div key={color} className="glass p-3 rounded-xl">
                    <div className="text-xs text-white/40 uppercase tracking-wider mb-2">{color === 'white' ? '♔ Белые' : '♚ Чёрные'}</div>
                    <div className="flex flex-wrap gap-1.5">
                      {(Object.entries(analysis.summary[color]) as [MoveClassification, number][]).filter(([, c]) => c > 0).map(([cls, count]) => {
                        ...
                        return <div key={cls} className="flex items-center gap-1 px-1.5 py-0.5 rounded text-xs" style={{ background: cfg.bgColor, color: cfg.color }}>
                          <span>{cfg.icon}</span><span className="font-bold">{count}</span><span className="hidden sm:inline">{cfg.label}</span>
                        </div>;
                      })}
                    </div>
                  </div>
              ...
              <div className="glass p-3 rounded-xl mb-6 cursor-pointer">
                <div className="text-xs text-white/40 uppercase tracking-wider mb-2">График оценки</div>
                  ...
                  <motion.div className="flex items-start gap-2 mb-2 px-3 py-2 rounded-xl"
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-semibold" style={{ color: CLASSIFICATION_CONFIG[currentAnalyzedMove.classification].color }}>
                        {CLASSIFICATION_CONFIG[currentAnalyzedMove.classification].icon} {currentAnalyzedMove.san} — {CLASSIFICATION_CONFIG[currentAnalyzedMove.classification].label}
                      </div>
            ...
            {analysis && (() => {
              ...
              return (
                <div className="glass rounded-xl p-3 mb-3">
                  <div className="text-xs text-white/40 uppercase tracking-wider mb-2">⚠️ Ваши ошибки ({mistakes.length})</div>
                  <div className="space-y-1 mb-3">
                    {mistakes.slice(0, 5).map((m: any) => (
                      <div key={m.index} onClick={() => goToMove(m.index)}
                        ...
                        <span className="text-white/60">Ход {m.moveNumber}: {m.san}</span>
            ...
            <div className="flex-1 min-w-0">
              <div className="glass rounded-xl overflow-hidden flex flex-col" style={{ maxHeight: boardWidth + 80 }}>
                ...
                <div className="flex-1 overflow-y-auto p-2">
                        ...
                        return (
                          <div key={i} className="flex gap-0.5 text-sm font-mono">
                            ...
                            <button onClick={() => goToMove(wI)}
                              ...
                              {wA && <span className="text-[10px] flex-shrink-0">{CLASSIFICATION_CONFIG[wA.classification].icon}</span>}
                            ...
                            {bM && (
                              <button onClick={() => goToMove(bI)}
                                ...
                                {bA && <span className="text-[10px] flex-shrink-0">{CLASSIFICATION_CONFIG[bA.classification].icon}</span>}
                  ...
                  ) : <p className="text-white/30 text-sm text-center py-8">Нет ходов</p>}
                </div>
              </div>
            </div>
          </div>
