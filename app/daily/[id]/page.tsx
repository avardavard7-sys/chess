...
export default function DailyGamePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_OUT') {
        setUserId(null);
      } else if (event === 'SIGNED_IN' && session?.user && !currentUid) {
        currentUid = session.user.id;
        setUserId(currentUid);
        loadGame(currentUid);
      }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <motion.div className="glass p-4 rounded-xl mb-4 flex items-center gap-4"
            ...
            <div className="flex-1">
              <div className="text-sm font-bold text-yellow-400">{t('sidebar_correspondence')}</div>
            ...
            <div className="text-right">
              {gameOver ? (
                <span className="text-xs px-2 py-1 rounded-full bg-white/10 text-white/50">{t('tournament_finished')}</span>
              ) : isMyTurn ? (
                <span className="text-xs px-2 py-1 rounded-full bg-green-400/15 text-green-400 font-semibold">{t('your_turn')}</span>
              ) : (
                <span className="text-xs px-2 py-1 rounded-full bg-white/5 text-white/30">{t('opponent_turn')}</span>
              )}
            </div>
          </motion.div>
              ...
              <div className="glass rounded-xl overflow-hidden flex flex-col" style={{ maxHeight: boardWidth }}>
                ...
                <div className="flex-1 overflow-y-auto p-2">
                  {moveHistory.length > 0 ? (
                    ...
                    <p className="text-white/30 text-sm text-center py-8">{t('loading')}</p>
                  )}
                </div>
