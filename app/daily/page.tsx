      ...
      <main className="pt-24 pb-12 px-4">
        <div className="max-w-2xl mx-auto">
          <motion.div className="text-center mb-8" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-3xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif", color: '#f59e0b' }}>{t('sidebar_correspondence')}</h1>
          ...
          {!loading && games.length === 0 && (
            <div className="glass p-10 rounded-2xl text-center">
              ...
              <p className="text-white/20 text-sm mt-2">{t('create_or_code')}</p>
            </div>
          )}
              ...
              {games.map(g => {
                ...
                return (
                  <motion.div key={g.id}
                    ...
                    {isActive && !isMyTurn && <span className="text-xs text-blue-400 px-2 py-1 rounded-full bg-blue-400/10 flex-shrink-0">{t('waiting_move')}</span>}
